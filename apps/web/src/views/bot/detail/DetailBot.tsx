...
export const DetailBot = (props: Props) => {
  const { bot, botName, visible, setVisible } = props;
  ...
  useEffect(() => {
    if (bot._id) {
      ...
      botService.getBotLog(bot._id)
      .then(res => setLogs(res.data))
      .catch(err => setLogs(err));

      botService.getBotHistory(bot._id)
      .then(res => {
        if (mounted) {
  ...
  }, [bot, bot._id, bot.blockchain._id, bot.node._id, bot.dex._id, bot.token, bot.wallet._id, bot.coin]);
