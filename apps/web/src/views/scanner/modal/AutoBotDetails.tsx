...
import { IAutoBot, ITokenDetailDto, ITokenDetailReqDto, IBotHistory, ETradingThread, ESocketType, ISocketData, IBotStatistics } from '../../../types';
...
export const AutoBotDetails = (props: Props) => {
  ...
  useEffect(() => {
    ws.socket.onmessage = (message: IMessageEvent) => {
      ...
      if (data.type === ESocketType.AUTO_BOT_WITHDRAW) {
        if (data.data.botId === botId) {
          setIsWithdraw(false);
          data.data.success && showNotification("Succeed", "success");
          !data.data.success && showNotification("Failed", "error");
        }
      } else if(data.type === ESocketType.AUTO_BOT_SUB_WALLET_INFO){
        if (data.data.botId === botId) {
          setSubWalletInfo(data.data.walletInfo);
        }
      }
    }
  }, [botId, visible]);
  ...
  useEffect(() => {
    ...
    ws.wsAction(ESocketType.AUTO_BOT_SUB_WALLET_INFO, {
      botId: botId
    });
  }, [bot, botId]);
  ...
  const handleWithdraw = () => {
    setIsWithdraw(true);
    ws.wsAction(ESocketType.AUTO_BOT_WITHDRAW, {
      botId: botId
    });
  }
