...
export class MaxGasPrice extends EventEmitter {
  ...
  constructor(blockchain: IStoredBlockchain, node: IStoredNode, wsServer: WebSocket.Server) {
    super();
        ...
        for (let i = 0; i < res.transactions.length; i++) {
          ...
          if (tx && tx?.gasPrice) {
            ...
            const socketData: ISocketData = {
              type: ESocketType.CHAIN_MAX_GAS_PRICE,
              data: {
                blockchainId: this.blockchain._id,
                blockchainName: this.blockchain.name,
                MaxGasPrice: this.getMaxGasPrice()
              }
            };
