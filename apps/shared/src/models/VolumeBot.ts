...
const schema: SchemaOf<IVolumeBot> = {
    ...
    isAutoSellingStrategy: { type: Boolean, required: false},
    autoSellingTxHash: [{ type: String, required: false }],
    alertAmountForPurchase: { type: Number, required: false},
    percentForAutoSelling: { type: Number, required: false},
...
const VolumeBotSchema: Schema = new Schema(schema);
